...
const MONGODB_URI = "mongodb://localhost:27017/cinema-app";
...
mongoose
  ...
  .then(() => {
    console.log("dsadsa");
    Movie.insertMany(movies).then((moviesInDB) => {
      moviesInDB.forEach((movies) => {
        console.log(`${movies.title} just added to the database`);
        // mongoose.disconnect();
      });
